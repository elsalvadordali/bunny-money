<script lang="ts" type="module">
  ...
  function acceptTransaction(i: number) {
    message = `Pay $${Math.abs(kid?.pending[i].amount)} ${
      kid && kid.pending[i].memo
    }?`
    boxOpen = true
    currentIndex = i
    ...
    if (kid) {
      ...
      if (toApprove) {
        ...
        kid.checkingAccount.balance =
          newTransaction.currentBalance || kid.checkingAccount.balance
        toast = 'payment sent!'
...
>
  {#if kid.pending && kid.pending.length > 0}
    <div
      class="bg-green rounded-xl p-2 mt-6 border-black border-2 w-5/6 max-w-lg shaded big-shade"
    >
      <h2 class="text-xl m-2">Pending requests</h2>
      {#each kid.pending as action, i (i)}
        {#if action.for != 'parent'}
          <div
            class="border-black border-b-2 grid grid-col-2 grid-row-3 pt-4 pb-4"
          >
            <p class="row-start-1 col-start-1 mb-2 bold">
              $ {Math.abs(action.amount).toFixed(2)}
            </p>

            <p class="row-start-1 col-start-2 mb-2">{action.date}</p>
            <p class="row-start-2 col-start-1 col-end-3 mb-2 italic">
              {action.memo}
            </p>
            <button
              on:click={() => acceptTransaction(i)}
              class="bg-yellow p-2 pr-4 pl-4 mr-2 rounded-md row-start-3 col-start-1 border-black border-2 mb-1 shaded hover:mb-0 hover:mt-1 action:mb-0 action:mt-1"
            >
              Accept
            </button>
            <button
              on:click={() => deleteTransaction(i)}
              class="bg-amber p-2 pr-4 pl-4 ml-2 rounded-md row-start-3 col-start-2 border-black border-2 mb-1 shaded"
            >
              Deny
            </button>
          </div>
