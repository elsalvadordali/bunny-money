...
import Toast from "../Toast.svelte";
    ...
    function transfer() {
        if (amount) {
            // Try using: Number.parseFloat instead, will give you NaN
            // 12,304.02
              // take out any non-number or period
            ...
            if (kid && amt) {
                if (checking2savings && kid.checkingAccount.balance >= amt) {
                    ...
                    kid.savingsAccount.transactions = [{date: convertDate2String(new Date()), memo: 'transfer from checking', amount: amt, currentBalance: kid.savingsAccount.balance}, ...kid.savingsAccount.transactions]
                } else if (!checking2savings && kid.savingsAccount.balance >= amt) {
                    kid.savingsAccount.balance -= amt
                    kid.checkingAccount.balance += amt
                    kid.checkingAccount.transactions = [{date: convertDate2String(new Date()), memo: 'transfer from savings', amount: amt, currentBalance: kid.checkingAccount.balance}, ...kid.checkingAccount.transactions]
                } else {
    ...
    const handle = (e: Event) => {
        ...
        return parseFloat(userInput) ? parseFloat(userInput) : false
    }
</script>
...
<div class="bg-green rounded-xl mb-4 mt-4 p-2">
    ...
    <div class='border-black border-2 rounded-xl p-2'>
        ...
        <div class="grid grid-col-3 grid-row-3 gap-4">
            ...
            <input type=number required id='amount' on:input={handle} value={amount} class="col-start-2 col-end-4 mb-1 w-13 row-start-2 bg-yellow rounded-md shaded big-shade" /> 
